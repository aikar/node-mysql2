...
function Connection(opts) {
  ...
  this.stream.setKeepAlive(true, 300000);
  this.stream.on('timeout', connection._handleTimeoutError.bind(this));
  ...
  if (!this.config.isServer) {
    ...
    handshakeCommand.on('end', function() {
      ...
      if (connection.config.timezone && connection.config.timezone !== 'local') {
        connection.query("SET time_zone = ?", [connection.config.timezone], function (err) {
          if (err) {
            connection.end();
...
Connection.prototype._handleTimeoutError = function() {
  ...
  err.code = 'ETIMEDOUT';
  err.syscall = wasConnected ? 'read' : 'connect';
