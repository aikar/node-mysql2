...
function Pool(options) {
  ...
  this.debug = options.config.debugLogger;
...
Pool.prototype.getConnection = function(cb) {
  ...
  if (this._freeConnections.length > 0) {
    ...
    this.debug && this.debug("Got free connection");
  ...
  if (
    ...
    this.debug && this.debug("Created Connection");
...
Pool.prototype.releaseConnection = function(connection) {
  this.debug && this.debug("Releasing Connection");
  var cb;
...
Pool.prototype.end = function(cb) {
  this.debug && this.debug("Closing Pool - " + this._allConnections.length + " connections");
  this._closed = true;
  ...
  var endCB = function(err) {
    ...
    if (err || ++closedConnections >= numConnections) {
      this.debug && this.debug("Pool Close Complete");
      calledBack = true;
      cb(err);
      return;
    }
...
Pool.prototype._manageExpiredTimer = function() {
  ...
  if (hasExtra && !this._expiredTimer) {
    this.debug && this.debug("Starting Expire Timer");
    ...
    this.debug && this.debug("Stopping Expire Timer");
...
Pool.prototype._closeIdleConnections = function() {
  ...
  this.debug && this.debug("Idle Check on " + length + " Connections to close up to " + numExtra + " extra connections");
...
Pool.prototype._removeConnection = function(connection) {
  this.debug && this.debug("Removing Connection");
...
Pool.prototype._closeConnection = function (connection, cb) {
  this.debug && this.debug("Closing Connection");
